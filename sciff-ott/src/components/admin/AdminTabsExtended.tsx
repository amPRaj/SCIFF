...
export const SubscriptionsTab: React.FC<TabProps> = ({ subscriptions, schools, categories, onRefresh }) => {
  ...
  const handleDeleteSubscription = async (subscriptionId: string, schoolName: string, categoryName: string) => {
    if (window.confirm(`Are you sure you want to delete the subscription for "${schoolName}" in category "${categoryName}"? This action cannot be undone.`)) {
      try {
        const { error } = await supabase
          .from('school_subscriptions')
          .delete()
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        <button 
                          ...
                          className={`${sub.active ? 'text-red-400 hover:text-red-300' : 'text-green-400 hover:text-green-300'} p-2 rounded-lg hover:bg-gray-700 transition-colors`}
                          title={sub.active ? "Deactivate Subscription" : "Activate Subscription"}
                        >
                        ...
                        <button 
                          onClick={() => handleDeleteSubscription(sub.id, sub.school?.name || 'Unknown School', sub.category?.name || 'Unknown Category')}
                          className="text-red-400 hover:text-red-300 p-2 rounded-lg hover:bg-gray-700 transition-colors"
